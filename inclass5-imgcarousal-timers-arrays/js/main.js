...
// stop slide show
document.querySelector('.carousel>img').addEventListener('mouseover', (event) => {
  clearInterval(slideshowInterval);
});

// resume on mouse out
document.querySelector('.carousel>img').addEventListener('mouseout', (event) => {
  setInterval(() => {
    currentImg += 1;
    if (currentImg === images.length) {
      currentImg = 0;
